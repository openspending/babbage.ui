...
export class RadarChartComponent extends events.EventEmitter {
  ...
  build(endpoint, cube, params, wrapper, colorSchema) {
    ...
    function RadarChart (wrapper, allData, legendOptions, options) {
      ...
      var data = _.map(allData, function(datum){return datum.axes;});
...
//Initiate Legend
    ...
    api.aggregate(endpoint, cube, params).then((data) => {
      ...
      var dim0vals = _.uniq(_.map(cells, function(cell){return cell.dimensions[0].nameValue;}));
      var dim1vals = _.uniq(_.map(cells, function(cell){return cell.dimensions[1].nameValue;}));

      var values =
          _.map(dim0vals, function(dim0val) {
            var axes =  _.map(_.filter(cells, function(cell) {
              return cell.dimensions[0].nameValue == dim0val;
            }), function(cell) {
              return {axis:cell.dimensions[1].nameValue, value: cell.measures[0].value}
            });
            ...
            return {className: dim0val , axes: axes}
          });
